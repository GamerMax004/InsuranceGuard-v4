...
DEFAULT_CONFIG = {
    ...
    "insurance_types": {
        "Krankenversicherung (Privat)": {
            "price": 10000.00, "role": "Krankenversicherung (Privat)",
            "auszahlung_limit": 20000.00, "enabled": True
        },
        "Haftpflichtversicherung": {
            "price": 10000.00, "role": "Haftpflichtversicherung",
            "auszahlung_limit": 20000.00, "enabled": True
        },
        "Hausratversicherung": {
            "price": 10000.00, "role": "Hausratversicherung",
            "auszahlung_limit": 20000.00, "enabled": True
        },
        "Kfz-Versicherung": {
            "price": 7500.00, "role": "Kfz-Versicherung",
            "auszahlung_limit": 15000.00, "enabled": True
        },
        "Rechtsschutzversicherung": {
            "price": 5000.00, "role": "Rechtsschutzversicherung",
            "auszahlung_limit": 10000.00, "enabled": True
        },
        "Berufsunfähigkeitsversicherung": {
            "price": 10000.00, "role": "Berufsunfähigkeitsversicherung",
            "auszahlung_limit": 20000.00, "enabled": True
        },
        "Bußgeldversicherung": {
            "price": 10000.00, "role": "Bußgeldversicherung",
            "auszahlung_limit": 20000.00, "enabled": True
        }
    }
}
...
#   HILFSFUNKTIONEN
# ═══════════════════════════════════════════════════════
async def send_to_log_channel(guild: discord.Guild, embed: discord.Embed, view: Optional[discord.ui.View] = None):
...
def _render_embed_html(embed: discord.Embed) -> str:
    ...
    if embed.title:
        if embed.url:
            parts.append(f'<a href="{html_lib.escape(embed.url)}" target="_blank" class="embed-title-link">{_esc(embed.title)}</a>')
        else:
...
def generate_transcript_html(
    ...
    type_labels = {"schadensmeldung": "Schadensmeldung", "kundenkontakt": "Kundenkontakt"}
    ...
    for msg in messages:
        ...
        av = (str(msg.author.avatar.url).split("?")[0] + "?size=64") if msg.author.avatar \
...
async def generate_and_save_transcript(
    ...
    try:
        ...
        html_content = generate_transcript_html(channel.name, customer_name, ticket_type, messages)
        tid  = str(uuid.uuid4())
...
async def close_ticket_channel(
    ...
    if ticket_type == "schadensmeldung" and customer_id in data.get("schaden_historie", {}):
    ...
    # ── Close-Embed ──────────────────────────────────────────────────────────
    title_text = "Ticket automatisch geschlossen" if auto_close else "Ticket geschlossen"
    desc_parts = ["**Bearbeitung abgeschlossen.**" if not auto_close else "**Wegen Inaktivität automatisch geschlossen.**"]
    desc_parts.append(f"> Kanal wird in **5 Sekunden** gelöscht." if not auto_close else "")
    if duration_min:
    ...
    if not auto_close:
        close_embed.add_field(name="Geschlossen von", value=f"> {closer_mention}", inline=True)
    if transcript_url:
        close_embed.add_field(name="Transcript", value="> Über den Button unten abrufbar.", inline=False)

    view = discord.ui.View()
    if transcript_url:
        view.add_item(discord.ui.Button(label="Transcript anzeigen", url=transcript_url))
    ...
    if transcript_url:
        ...
        log_view.add_item(discord.ui.Button(label="Transcript anzeigen", url=transcript_url))
    ...
    add_log_entry("TICKET_GESCHLOSSEN", closer_id, {
        "customer_id": customer_id, "channel_name": channel.name,
        "auto_close": auto_close, "transcript_url": transcript_url or "—"
    })
...
async def customer_id_all_autocomplete(
    ...
    for cid, c in data["customers"].items():
        ...
        if not q or q in cid.lower() or q in rp.lower():
            choices.append(app_commands.Choice(name=f"[{status}] {rp}  ({cid})"[:100], value=cid))
        if len(choices) >= 25:
            break
    return choices
...
def _has_any_role(interaction: discord.Interaction, role_ids) -> bool:
    ...
    if not isinstance(role_ids, tuple):
        role_ids = (role_ids,)
    return any(
        (role := interaction.guild.get_role(rid)) is not None and role in interaction.user.roles
        for rid in role_ids
    )
...
def is_mitarbeiter(interaction: discord.Interaction) -> bool:
    return (
        _has_any_role(interaction, MITARBEITER_ROLE_ID) or
        _has_any_role(interaction, LEITUNGSEBENE_ROLE_ID) or
...
def build_error_embed(title: str, description: str, needed_permission: Optional[str] = None) -> discord.Embed:
    ...
    if needed_permission:
        e.add_field(name="Erforderliche Berechtigung", value=f"> `{needed_permission}`", inline=False)
    return e
...
def build_kundenakte_embed(customer_id: str, customer: dict) -> discord.Embed:
    ...
    embed.add_field(name="Abgeschlossene Versicherungen", value=ins_text or "> Keine", inline=False)
    embed.add_field(
...
async def update_customer_thread_backup(guild: discord.Guild, customer_id: str):
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            color=EMBED_COLOR, timestamp=get_now()
        )
...
MODUL_COMMANDS = {
    "versicherung": {
        ...
        "rechnung-ausstellen", "rechnung-archivieren", "mahnung-ausstellen",
...
async def ping(interaction: discord.Interaction):
    ...
    aus_pend   = sum(1 for az in data.get('pending_auszahlungen', {}).values() if az.get('status') == 'ausstehend')
    ...
    embed = discord.Embed(title="Systemstatus InsuranceGuard v4", color=color, timestamp=get_now())
    embed.add_field(name="Verbindung", value=status_val, inline=False)
    embed.add_field(name="Kunden", value=f"> - `{aktive}` Kunden\n> - `{archiviert}` ehem. Kunden", inline=True)
    embed.add_field(
        name="Offene Vorgänge",
        value=f"> - **`{off_re}`** Rechnungen\n> - **`{aus_pend}`** Auszahlungen\n> - **`{offene_tk}`** Tickets",
        inline=True
    )
    embed.add_field(name="Serverzeit", value=f"> `{get_now().strftime('%d.%m.%Y, %H:%M:%S Uhr')}`", inline=False)
    embed.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
...
#   /modul
# ═══════════════════════════════════════════════════════
@bot.tree.command(name="modul", description="[ADMIN] Versicherungsmodul aktivieren oder deaktivieren")
...
async def modul_cmd(interaction: discord.Interaction, aktion: str):
    if not is_admin(interaction):
        await interaction.response.send_message(
            embed=build_error_embed("Zugriff verweigert!", "Nur Administratoren können Module verwalten.", "Administrator"),
            ephemeral=True
        )
        return
    ...
    e = discord.Embed(
        ...
        color=EMBED_COLOR if aktiv else COLOR_WARNING, timestamp=get_now()
    )
    e.add_field(name="Aktuelle Module", value=f"> {icon} Versicherungsfirma", inline=False)
    e.add_field(name="Hinweis", value="> Sollten die Befehle nicht sofort erscheinen, lade die Discord-App neu.", inline=False)
    e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
    await interaction.followup.send(embed=e, ephemeral=True)
    add_log_entry("MODUL_GEAENDERT", interaction.user.id, {"modul": "versicherung", "aktion": aktion})

# ═══════════════════════════════════════════════════════
#   BACKUP & RELOAD
# ═══════════════════════════════════════════════════════
@bot.tree.command(name="backup", description="Erstellt ein Backup und sendet es als ZIP")
async def backup_download(interaction: discord.Interaction):
    ...
    try:
        ...
        file = discord.File(buf, filename=f"insurance_backup_{get_now().strftime('%Y%m%d_%H%M%S')}.zip")
        await interaction.followup.send("## Vollständiger Datenbank-Export", file=file, ephemeral=True)
    except Exception as e:
...
async def reload_backup(interaction: discord.Interaction, datei: discord.Attachment):
    ...
    if not datei.filename.endswith('.json'):
        await interaction.response.send_message(
            embed=build_error_embed("Falscher Dateityp!", "Nur `.json` Dateien sind erlaubt."), ephemeral=True
        )
        return
    await interaction.response.defer(ephemeral=True)
    try:
        ...
        if "customers" in json_obj and "logs" in json_obj:
            ...
            await interaction.followup.send(
                " Unbekanntes Dateiformat.", ephemeral=True
            )
    except Exception as e:
        await interaction.followup.send(f" Fehler: {e}", ephemeral=True)
...
async def kunden_suchen(interaction: discord.Interaction, suchbegriff: str):
    ...
    try:
        ...
        treffer = [
            ...
            if q in cid.lower() or q in c.get('rp_name','').lower()
            or q in c.get('hbpay_nummer','').lower() or q in c.get('economy_id','').lower()
        ]
        if not treffer:
            embed = discord.Embed(
            ...
            embed.add_field(name="Suchtipps", value="> - Kunden-ID: `VN-26123456`\n> - RP-Name: `Max Mustermann`\n> - Kartennummer oder Economy-ID", inline=False)
            embed.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
        ...
        for cid, c in treffer[:10]:
            ...
            versicherungen = "\n".join(f"> - {ins}" for ins in c.get('versicherungen', [])) or "> - Keine"
            embed.add_field(
                name=f"{c['rp_name']}",
                value=(
                    f">  - `{cid}`\n"
...
class KündigungSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        e.add_field(name="Zu kündigende Versicherungen", value="\n".join(f">  {ins}" for ins in self.values), inline=False)
        e.add_field(name="Beitragsänderung", value=f"> Wegfall: `-{wegfall:,.2f} €`\n> Neuer Beitrag: **`{neu_total:,.2f} €`**", inline=False)
...
async def versicherung_kündigen(interaction: discord.Interaction, customer_id: str):
    ...
    if customer_id not in data['customers']:
        await interaction.response.send_message(embed=build_error_embed("Nicht gefunden!", f"Keine Kundenakte mit ID `{customer_id}`."), ephemeral=True)
        return
    ...
    e.add_field(name="Aktueller Monatsbeitrag", value=f">  **`{customer.get('total_monthly_price',0):,.2f} €`**", inline=False)
    e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
    await interaction.response.send_message(embed=e, view=view, ephemeral=True)
    await view.wait()

    if not view.confirmed:
        await interaction.edit_original_response(embed=dvg_embed("Kündigung abgebrochen."), view=None)
        return
    ...
    new_total = sum(ins_types.get(ins,{}).get('price',0) for ins in data['customers'][customer_id]['versicherungen'])
    ...
    if member:
        for ins in to_cancel:
            role = discord.utils.get(interaction.guild.roles, name=ins_types.get(ins,{}).get("role", ins))
    ...
    if thread_id:
        try:
            ...
            if thread:
                ...
                v.add_field(name="Gekündigte Versicherungen", value="\n".join(f">  {ins}" for ins in to_cancel), inline=False)
                v.add_field(name="Neuer Monatsbeitrag", value=f"> `{new_total:,.2f} €`", inline=True)
                v.add_field(name="Durchgeführt von", value=f"> {interaction.user.mention}", inline=True)
    ...
    if member:
        dm = dvg_embed("Versicherungskündigung", "> Folgende Versicherungen wurden entfernt.")
        dm.add_field(name="Gekündigte Versicherungen", value="\n".join(f">  {ins}" for ins in to_cancel), inline=False)
        dm.add_field(name="Neuer Monatsbeitrag", value=f"> `{new_total:,.2f} €`", inline=False)
    ...
    add_log_entry("VERSICHERUNG_GEKUENDIGT", interaction.user.id, {"customer_id": customer_id, "gekuendigte_versicherungen": list(to_cancel), "neuer_beitrag": new_total})
    log_e = dvg_embed("Versicherung(en) gekündigt!")
    log_e.add_field(name="Versicherungsnehmer", value=f"> {customer['rp_name']}\n> `{customer_id}`", inline=False)
    log_e.add_field(name="Gekündigt", value="\n".join(f">  {ins}" for ins in to_cancel), inline=False)
    log_e.add_field(name="Neuer Beitrag", value=f"> `{new_total:,.2f} €`", inline=True)
    log_e.add_field(name="Durchgeführt von", value=f"> {interaction.user.mention}", inline=True)
    log_e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
    await send_to_log_channel(interaction.guild, log_e)

    s = dvg_embed("Versicherung(en) gekündigt!", "> Die Kündigung wurde erfolgreich durchgeführt.")
    s.add_field(name="Versicherungsnehmer", value=f"> {customer['rp_name']}\n> `{customer_id}`", inline=False)
    s.add_field(name="Gekündigt", value="\n".join(f">  {ins}" for ins in to_cancel), inline=False)
    s.add_field(name="Neuer Monatsbeitrag", value=f">  **`{new_total:,.2f} €`**", inline=False)
...
async def rechnungen_Übersicht(interaction: discord.Interaction, customer_id: Optional[str] = None, nur_offen: bool = True):
    ...
    try:
        if customer_id and customer_id not in data['customers']:
            ...
            return
        gefiltert = {
            inv_id: inv for inv_id, inv in data['invoices'].items()
            if (not customer_id or inv['customer_id'] == customer_id)
            and (not nur_offen or not inv.get('paid', False))
        }
        if not gefiltert:
            await interaction.followup.send(embed=dvg_embed("Keine Rechnungen", "> Keine Rechnungen gefunden."), ephemeral=True)
            return

        sorted_inv = sorted(gefiltert.items(), key=lambda x: x[1].get('created_at',''), reverse=True)
        embed = discord.Embed(
        ...
        embed.add_field(name="Offene Summe", value=f"> **`{total_offen:,.2f} €`**", inline=False)
        embed.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
        await interaction.followup.send(embed=embed, ephemeral=True)
    except Exception as e:
...
async def set_steuer(interaction: discord.Interaction, prozent: float):
    ...
    if not (0 <= prozent <= 100):
        await interaction.response.send_message(embed=build_error_embed("Ungültiger Wert!", "0–100."), ephemeral=True)
        return
    config["steuer_prozent"] = prozent
    save_config(config)
    e = discord.Embed(title="Steuersatz aktualisiert!", description=f"> Neuer Steuersatz: **`{prozent}%`**", color=EMBED_COLOR)
    e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
    await interaction.response.send_message(embed=e, ephemeral=True)
...
@app_commands.describe(name="Name der Versicherung", preis="Monatlicher Beitrag in €", auszahlungslimit="Maximaler Auszahlungsbetrag in €", rollenname="Discord-Rolle (Standard: gleich wie Name)")
async def add_versicherung(interaction: discord.Interaction, name: str, preis: float, auszahlungslimit: float, rollenname: str = ""):
    ...
    if name in config["insurance_types"]:
        await interaction.response.send_message(embed=build_error_embed("Bereits vorhanden!", f"`{name}` existiert bereits."), ephemeral=True)
        return
...
@app_commands.describe(name="Name der Versicherung", neuer_preis="Neuer Beitrag (-1 = keine Änderung)", neues_limit="Neues Auszahlungslimit (-1 = keine Änderung)", aktiviert="Aktivieren/Deaktivieren")
async def edit_versicherung(interaction: discord.Interaction, name: str, neuer_preis: float = -1.0, neues_limit: float = -1.0, aktiviert: Optional[bool] = None):
    ...
    if name not in config["insurance_types"]:
        await interaction.response.send_message(embed=build_error_embed("Nicht gefunden!", f"Keine Versicherung `{name}`."), ephemeral=True)
        return
    changes = []
    if neuer_preis >= 0.0:
...
async def list_versicherungen(interaction: discord.Interaction):
    ...
    e.add_field(name="Steuersatz", value=f"> `{config.get('steuer_prozent', 3.0)}%`", inline=False)
    for name, v in config.get("insurance_types", {}).items():
        icon = "" if v.get("enabled", True) else ""
        e.add_field(name=f"{icon} {name}", value=f"> Preis: `{v['price']:,.2f} €/Mo`\n> Limit: `{v['auszahlung_limit']:,.2f} €`\n> Rolle: `{v['role']}`", inline=True)
    e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
    await interaction.response.send_message(embed=e, ephemeral=True)
...
async def set_channels(
    ...
    if not any([log_channel, kundenkontakt_panel, schadensmeldung_panel, auszahlung_kanal,
                dokumentation_kanal, kundenkontakt_kategorie, schadensmeldung_kategorie]):
        ...
        e.add_field(name="Auszahlung",     value=ch(config.get("auszahlung_channel_id")),        inline=True)
        e.add_field(name="Dokumentation",  value=ch(config.get("dokumentation_channel_id")),     inline=True)
    ...
    if kundenkontakt_panel:
        try:
            ...
            embed_kk = dvg_embed("Kundenkontakt", "Liebe Mitarbeiter:innen,\n> hier können sie mit unseren Kunden Kontakt aufnehmen.")
            embed_kk.add_field(name="Wie funktioniert das System?", value="> 1. Klicken Sie unten auf den Button!\n> 2. Geben Sie die Kunden-ID ein!\n> 3. Beschreiben Sie einen detaillierten Kontaktgrund!\n> 4. Ein privater Ticket-Channel wird erstellt!", inline=False)
            embed_kk.add_field(name="Was muss ich beachten?", value="> - Gültige **Kunden-ID** erforderlich!\n> - Kontaktgrund **detailliert** beschreiben!\n> - Nur für **Mitarbeiter** und **Leitungsebene**!\n> - Pro Mitarbeiter nur **ein aktives Ticket** gleichzeitig!", inline=False)
    ...
    if schadensmeldung_panel:
        try:
            ...
            embed_sm = dvg_embed("Schadensmeldung", "Liebe Versicherungsnehmer:innen,\n> hier können sie Schadensmeldungen einreichen.")
            embed_sm.add_field(name="Wie funktioniert das System?", value="> 1. Klicken Sie auf den Button unten!\n> 2. Geben Sie Ihre Kunden-ID ein!\n> 3. Füllen Sie das Formular aus!\n> 4. Ein Schadensfall-Ticket wird erstellt!", inline=False)
            embed_sm.add_field(name="Welche Angaben sind erforderlich?", value="> - **Kunden-ID**\n> - **Geschädigter** (RP-Name)\n> - **Täter** (RP-Name)\n> - **Vorfallbeschreibung**\n> - **Rechnung/Nachweis**", inline=False)
            # Voraussichtliche Bearbeitungszeit aus ticket_stats berechnen
            sm_stats   = [s for s in data.get("ticket_stats", [])
            ...
            embed_sm.add_field(name="Hinweis", value="> Pro Kunden nur **eine aktive Schadensmeldung** gleichzeitig!", inline=False)
    ...
    if changes:
        e.add_field(name="Gespeicherte Einstellungen", value="\n".join(f"> {c}" for c in changes), inline=False)
    if panel_results:
        e.add_field(name="Panel-Status", value="\n".join(f"> {r}" for r in panel_results), inline=False)
    e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
    await interaction.followup.send(embed=e, ephemeral=True)
...
class InsuranceSelect(discord.ui.Select):
    def __init__(self):
        ins_types = get_insurance_types()
        options   = [discord.SelectOption(label=ins, description=f"Monatsbeitrag: {info['price']:,.2f} €", value=ins) for ins, info in ins_types.items()]
        super().__init__(placeholder="Gewünschte Versicherungen auswählen...", min_values=1, max_values=len(options), options=options, custom_id="insurance_select")
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        preview   = "\n".join(f"> - {ins} — {ins_types[ins]['price']:,.2f} €" for ins in self.values)
        e = discord.Embed(title="Versicherungen ausgewählt!", description=f"{preview}\n\n**Gesamtbeitrag:** `{total:,.2f} €`", color=EMBED_COLOR)
...
class InsuranceView(discord.ui.View):
    def __init__(self):
        ...
        btn = discord.ui.Button(label="Kundenakte erstellen", style=discord.ButtonStyle.green, custom_id="confirm_insurance", disabled=True)
...
class AddInsuranceSelect(discord.ui.Select):
    def __init__(self, existing_insurances: list):
        ins_types = get_insurance_types()
        options   = [discord.SelectOption(label=ins, description=f"Monatsbeitrag: {info['price']:,.2f} €", value=ins) for ins, info in ins_types.items() if ins not in existing_insurances]
        super().__init__(placeholder="Neue Versicherung(en) wählen...", min_values=1, max_values=len(options), options=options, custom_id="add_insurance_select")
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        ins_types = get_insurance_types()
        total = sum(ins_types[ins]["price"] for ins in self.values)
        e = discord.Embed(title="Neue Versicherungen ausgewählt!", description="\n".join(f"> - {ins} — {ins_types[ins]['price']:,.2f} €" for ins in self.values) + f"\n\n**Zusätzlicher Beitrag:** `{total:,.2f} €`", color=EMBED_COLOR)
        await interaction.response.edit_message(embed=e, view=view)
...
@app_commands.describe(forum_channel="Forum-Channel für Kundenakten", user="Discord-User des Versicherungsnehmers", rp_name="RP-Name", hbpay_nummer="HBpay Kontonummer", economy_id="Economy-ID")
async def create_customer(interaction: discord.Interaction, forum_channel: discord.ForumChannel, user: discord.Member, rp_name: str, hbpay_nummer: str, economy_id: str):
    if not is_mitarbeiter(interaction):
        await interaction.response.send_message(embed=build_error_embed("Zugriff verweigert!", "Nur Mitarbeiter.", "Mitarbeiter / Leitungsebene"), ephemeral=True)
        return
    view = InsuranceView()
    select_embed = dvg_embed("Versicherungen auswählen!", "> Bitte wählen Sie die gewünschten Versicherungen aus dem Dropdown-Menü.")
    await interaction.response.send_message(embed=select_embed, view=view, ephemeral=True)
    await view.wait()
    if not view.confirmed:
        await interaction.edit_original_response(embed=dvg_embed("Zeitüberschreitung!"), view=None)
        return
    ins_list = view.children[0].values
    if not ins_list:
        await interaction.edit_original_response(embed=dvg_embed("Keine Auswahl!"), view=None)
        return
    ...
    try:
        ...
        customer_data = {
            "rp_name": rp_name, "hbpay_nummer": hbpay_nummer, "economy_id": economy_id,
            "versicherungen": ins_list, "total_monthly_price": total_price,
            "thread_id": None, "discord_user_id": user.id,
            "created_at": get_now().isoformat(), "created_by": interaction.user.id,
            "status": "aktiv", "auszahlungen": {}
        }
        embed  = build_kundenakte_embed(customer_id, customer_data)
        thread = await forum_channel.create_thread(name=f" {customer_id} | {rp_name}", content="", embed=embed)
        ...
        add_log_entry("KUNDENAKTE_ERSTELLT", interaction.user.id, {"customer_id": customer_id, "rp_name": rp_name, "versicherungen": ins_list, "total_price": total_price, "thread_id": thread.thread.id, "discord_user_id": user.id})
        await update_customer_thread_backup(interaction.guild, customer_id)

        log_e = dvg_embed("Neue Kundenakte erstellt!")
        log_e.add_field(name="Versicherungsnehmer", value=f"> {rp_name}\n> `{customer_id}`", inline=False)
        log_e.add_field(name="Discord", value=f"> {user.mention}", inline=True)
        log_e.add_field(name="Monatsbeitrag", value=f"> `{total_price:,.2f} €`", inline=True)
        log_e.add_field(name="Aussteller", value=f"> {interaction.user.mention}", inline=False)
        log_e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
        await send_to_log_channel(interaction.guild, log_e)

        s = dvg_embed("Kundenakte erfolgreich angelegt!")
        s.add_field(name="Informationen", value=f">  `{customer_id}`\n>  {thread.thread.mention}\n>  `{total_price:,.2f} €`\n>   DM gesendet!", inline=False)
        s.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
        await interaction.edit_original_response(embed=s, view=None)
    except Exception as e:
        logger.error(f"Kundenakte Fehler: {e}", exc_info=True)
        await interaction.edit_original_response(embed=discord.Embed(title="Fehler!", description=str(e), color=EMBED_COLOR), view=None)
...
async def add_insurance_to_customer(interaction: discord.Interaction, customer_id: str):
    if not is_mitarbeiter(interaction):
        ...
        return
    if customer_id not in data['customers']:
        await interaction.response.send_message(embed=build_error_embed("Nicht gefunden!", f"Keine Akte mit ID `{customer_id}`."), ephemeral=True)
        return
    customer  = data['customers'][customer_id]
    existing  = customer.get("versicherungen", [])
    ins_types = get_insurance_types()
    if not [ins for ins in ins_types if ins not in existing]:
        await interaction.response.send_message(embed=dvg_embed("Alle Versicherungen abgeschlossen!"), ephemeral=True)
        return
    view = AddInsuranceView(existing)
    e = discord.Embed(title="Versicherung nachbuchen", description=f"Kunde: **{customer['rp_name']}** (`{customer_id}`)\n\nBitte wähle die hinzuzufügenden Versicherungen.", color=EMBED_COLOR)
    await interaction.response.send_message(embed=e, view=view, ephemeral=True)
    await view.wait()
    if not view.confirmed:
        await interaction.edit_original_response(embed=dvg_embed("Abgebrochen."), view=None)
        return
    ...
    new_total = sum(ins_types[ins]["price"] for ins in data['customers'][customer_id]['versicherungen'])
    data['customers'][customer_id]['total_monthly_price'] = new_total
    save_data(data)
    await update_forum_thread_embed(interaction.guild, customer_id, data['customers'][customer_id])
    member = interaction.guild.get_member(customer['discord_user_id'])
    if member:
        for ins in new_insurances:
            role = discord.utils.get(interaction.guild.roles, name=ins_types[ins]["role"])
            if not role:
                role = await interaction.guild.create_role(name=ins_types[ins]["role"], color=discord.Color.from_rgb(44, 62, 80))
            await member.add_roles(role)
        dm = dvg_embed("Versicherungsänderung", "Neue Versicherungen wurden hinzugefügt.")
        dm.add_field(name="Neue Versicherungen", value="\n".join(f"> - {ins}" for ins in new_insurances), inline=False)
        dm.add_field(name="Neuer Monatsbeitrag", value=f"> `{new_total:,.2f} €`", inline=False)
        dm.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
        try:
            await member.send(embed=dm)
    ...
    s.add_field(name="Neue Versicherungen", value="\n".join(f"> - {ins}" for ins in new_insurances), inline=False)
    s.add_field(name="Neuer Monatsbeitrag", value=f"> `{new_total:,.2f} €`", inline=False)
    s.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
    await interaction.edit_original_response(embed=s, view=None)
...
async def create_invoice(interaction: discord.Interaction, customer_id: str, channel: discord.TextChannel):
    ...
    try:
        ...
        embed = discord.Embed(title=f"Versicherungsrechnung - {get_now().strftime('%d.%m.%Y')}", description="Dies ist eine Zahlungsaufforderung für Ihre Versicherungsbeiträge!", color=EMBED_COLOR, timestamp=get_now())
        ...
        embed.add_field(name="Zahlungsmethoden", value=f">  - `{customer['hbpay_nummer']}`\n>  - `{customer['economy_id']}`", inline=False)
        ins_details = "\n".join(f"> {ins}\n> - `{ins_types.get(ins,{}).get('price',0.0):,.2f} €`" for ins in customer['versicherungen'])
        embed.add_field(name="Abgeschlossene Versicherungen", value=ins_details, inline=False)
        ...
        embed.add_field(name="Rechnungsbetrag (Brutto)", value=f" **`{betrag_brutto:,.2f} €`**", inline=False)
        embed.add_field(name="Status: Zahlung ausstehend!", value=f"> Bis zum **{due_date.strftime('%d.%m.%Y')}** bezahlen.", inline=False)
        embed.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)

        message = await channel.send(embed=embed)
        data['invoices'][invoice_id] = {
            "customer_id": customer_id, "betrag": betrag_brutto, "betrag_netto": betrag_netto,
            "steuer": steuer, "steuer_prozent": steuer_prozent, "original_betrag": betrag_brutto,
            "paid": False, "message_id": message.id, "channel_id": channel.id,
            "due_date": due_date.isoformat(), "reminder_count": 0,
            "created_at": get_now().isoformat(), "created_by": interaction.user.id
        }
        save_data(data)
        add_log_entry("RECHNUNG_ERSTELLT", interaction.user.id, {"invoice_id": invoice_id, "customer_id": customer_id, "betrag_brutto": betrag_brutto})
        await update_customer_thread_backup(interaction.guild, customer_id)
        ...
        s.add_field(name="Fällig", value=f"> {due_date.strftime('%d.%m.%Y')}", inline=True)
        s.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
...
async def archive_invoice(interaction: discord.Interaction, invoice_id: str):
    ...
    try:
        ...
        if invoice.get('paid', False):
            await interaction.followup.send(embed=dvg_embed("Bereits archiviert!"), ephemeral=True)
            return
        ...
        data['invoices'][invoice_id].update({'paid': True, 'paid_by': interaction.user.id, 'paid_at': get_now().isoformat(), 'archived': True, 'reminder_count': 0})
        ...
        if thread_id:
            try:
                thread = interaction.guild.get_thread(thread_id) or await interaction.guild.fetch_channel(thread_id)
                if thread:
                    ae = dvg_embed("Archivierte Rechnung", "Bezahlt. Auszahlungslimit zurückgesetzt.")
                    ae.add_field(name="Rechnungs-Nr.", value=f"> `{invoice_id}`", inline=True)
                    ae.add_field(name="Betrag", value=f"> `{invoice['betrag']:,.2f} €`", inline=True)
                    ae.add_field(name="Archiviert von", value=f"> {interaction.user.mention}", inline=False)
                    ae.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
                    await thread.send(embed=ae)
            except Exception as ex:
                logger.error(f"Thread Eintrag Fehler: {ex}")

        add_log_entry("RECHNUNG_ARCHIVIERT", interaction.user.id, {"invoice_id": invoice_id, "customer_id": customer_id, "betrag": invoice['betrag']})
        await update_customer_thread_backup(interaction.guild, customer_id)

        s = discord.Embed(title="Rechnung archiviert!", description=f"`{invoice_id}` als bezahlt markiert.", color=EMBED_COLOR)
...
async def issue_manual_reminder(interaction: discord.Interaction, invoice_id: str):
    ...
    try:
        ...
        invoice = data['invoices'][invoice_id]
        if invoice.get('paid', False):
            await interaction.followup.send(embed=dvg_embed("Bereits bezahlt!"), ephemeral=True)
            return
        ...
        save_data(data)
        await send_reminder(invoice_id, data['invoices'][invoice_id], reminder_count, surcharge_pct)
        await update_customer_thread_backup(interaction.guild, invoice['customer_id'])

        s = discord.Embed(title=f"{reminder_count}. Mahnung ausgestellt!", description=f"Rechnung `{invoice_id}`", color=EMBED_COLOR)
        s.add_field(name="Neuer Betrag", value=f"> `{data['invoices'][invoice_id]['betrag']:,.2f} €`", inline=True)
        if surcharge_pct > 0:
...
async def send_reminder(invoice_id: str, invoice_data: dict, reminder_number: int, surcharge_percent: int):
    try:
        for guild in bot.guilds:
            ...
            surcharge_tx = f" (+{surcharge_percent}% Mahngebühr)" if surcharge_percent > 0 else ""
            embed = discord.Embed(title=f"{reminder_number}. Mahnung", description=f"Die Rechnung `{invoice_id}` ist überfällig!", color=EMBED_COLOR if reminder_number < 3 else COLOR_ERROR, timestamp=get_now())
            embed.add_field(name="Rechnungs-Nr.", value=f"> `{invoice_id}`\n> Mahnstufe: {reminder_number}", inline=False)
            embed.add_field(name="Betrag", value=f"> Ursprünglich: `{invoice_data['original_betrag']:,.2f} €`\n> **Aktuell: `{invoice_data['betrag']:,.2f} €`**{surcharge_tx}", inline=False)
            embed.set_footer(text="Bitte begleichen Sie den Betrag umgehend • Copyright © InsuranceGuard v4", icon_url=FOOTER_ICON)
            await channel.send(member.mention if member else "", embed=embed)
            add_log_entry(f"MAHNUNG_{reminder_number}", 0, {"invoice_id": invoice_id, "customer_id": invoice_data['customer_id'], "surcharge": surcharge_percent, "neuer_betrag": invoice_data['betrag']})
            break
    except Exception as e:
...
async def archive_customer(interaction: discord.Interaction, customer_id: str):
    ...
    try:
        ...
        if customer.get('status') == 'archiviert':
            await interaction.followup.send(embed=dvg_embed("Bereits archiviert!"), ephemeral=True)
            return
        data['customers'][customer_id].update({'status': 'archiviert', 'archived_at': get_now().isoformat(), 'archived_by': interaction.user.id})
        save_data(data)
        ...
        if thread_id:
            try:
                thread = interaction.guild.get_thread(thread_id) or await interaction.guild.fetch_channel(thread_id)
                if thread:
                    await thread.edit(name=f" [ARCHIV] {customer_id} | {customer['rp_name']}")
                    ae = dvg_embed("Akte archiviert!", "Diese Kundenakte wurde archiviert.")
                    ae.add_field(name="Archiviert von", value=f"> {interaction.user.mention}", inline=True)
                    ae.add_field(name="Datum", value=f"> {get_now().strftime('%d.%m.%Y • %H:%M Uhr')}", inline=True)
                    ae.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
                    await thread.send(embed=ae)
            except Exception as ex:
        ...
        if member:
            for insurance in customer.get('versicherungen', []):
                role = discord.utils.get(interaction.guild.roles, name=ins_types.get(insurance, {}).get("role", insurance))
                if role and role in member.roles:
                    await member.remove_roles(role)
            kunden_role = discord.utils.get(interaction.guild.roles, name=KUNDEN_ROLE_NAME)
            if kunden_role and kunden_role in member.roles:
        ...
        s = discord.Embed(title="Akte archiviert!", description=f"Kundenakte `{customer_id}` archiviert und alle Rollen entfernt.", color=EMBED_COLOR)
        s.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
        await interaction.followup.send(embed=s, ephemeral=True)
    except Exception as e:
        logger.error(f"Archivieren Fehler: {e}", exc_info=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not az_channel_id:
                await interaction.followup.send(" Auszahlungs-Kanal nicht konfiguriert.", ephemeral=True)
                return
            az_channel = interaction.guild.get_channel(az_channel_id)
            if not az_channel:
                await interaction.followup.send(" Auszahlungs-Kanal nicht gefunden.", ephemeral=True)
                return

            auszahlung_id   = generate_auszahlung_id()
            beschreibung_tx = self.beschreibung.value.strip() if self.beschreibung.value else "—"
            fkr_roles       = _get_roles(interaction.guild, FIRMENKONTOROLLE_ROLE_ID)
            ping_text       = " ".join(r.mention for r in fkr_roles) if fkr_roles else "@Firmenkontorolle"
            ...
            msg = await az_channel.send(content=f"{ping_text} — Neuer Auszahlungsantrag!", embed=embed, view=action_view)

            data["pending_auszahlungen"][auszahlung_id] = {
                "customer_id": self.customer_id, "versicherung": self.versicherung,
                "betrag": betrag_float, "beschreibung": beschreibung_tx,
                "requester_id": interaction.user.id, "message_id": msg.id,
                "channel_id": az_channel_id, "status": "ausstehend", "created_at": get_now().isoformat()
            }
            save_data(data)
            add_log_entry("AUSZAHLUNG_EINGEREICHT", interaction.user.id, {"auszahlung_id": auszahlung_id, "customer_id": self.customer_id, "versicherung": self.versicherung, "betrag": betrag_float})

            s = discord.Embed(title="Auszahlungsantrag eingereicht!", description=f"Antrag `{auszahlung_id}` wurde weitergeleitet.", color=EMBED_COLOR)
            s.add_field(name="Betrag", value=f"> `{betrag_float:,.2f} €`", inline=True)
            s.add_field(name="Versicherung", value=f"> `{self.versicherung}`", inline=True)
            s.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
            await interaction.followup.send(embed=s, ephemeral=True)
        except Exception as e:
...
class AuszahlungBestätigenModal(discord.ui.Modal, title="Auszahlung bestätigen – Nachweis"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            add_log_entry("AUSZAHLUNG_BESTAETIGT", self.confirmer.id, {"auszahlung_id": self.auszahlung_id, "customer_id": customer_id, "versicherung": versicherung, "betrag": betrag})
            await update_customer_thread_backup(self.guild, customer_id)

            s = discord.Embed(title="Auszahlung bestätigt!", description=f"Auszahlung `{self.auszahlung_id}` genehmigt.", color=EMBED_COLOR)
...
class AuszahlungActionView(discord.ui.View):
    ...
    async def abbrechen(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not is_firmenkontorolle(interaction):
            await interaction.response.send_message(embed=build_error_embed("Zugriff verweigert!", "Nur das Firmenkonto.", "Firmenkonto"), ephemeral=True)
            return
        az_id = self._get_az_id(interaction.message)
        pending = data.get("pending_auszahlungen", {}).get(az_id)
        if not pending or pending.get("status") != "ausstehend":
            ...
            return
        data["pending_auszahlungen"][az_id].update({"status": "abgelehnt", "abgelehnt_von": interaction.user.id, "abgelehnt_am": get_now().isoformat()})
        save_data(data)
        try:
        ...
        add_log_entry("AUSZAHLUNG_ABGELEHNT", interaction.user.id, {"auszahlung_id": az_id})
        await interaction.response.send_message(f" Antrag `{az_id}` wurde abgelehnt.", ephemeral=True)
...
async def auszahlung_einreichen(interaction: discord.Interaction, customer_id: str):
    ...
    customer = data['customers'][customer_id]
    if not customer.get("versicherungen"):
        await interaction.response.send_message(" Keine Versicherungen vorhanden.", ephemeral=True)
        return
    ins_types   = get_insurance_types()
    limits_text = ""
    for versicherung in customer.get("versicherungen", []):
...
class KundenkontaktView(discord.ui.View):
    ...
    @discord.ui.button(label="Kundenkontakt anfragen!", style=discord.ButtonStyle.secondary, custom_id="open_kundenkontakt")
...
class SchadensmeldungView(discord.ui.View):
    ...
    @discord.ui.button(label="Schadensmeldung einreichen!", style=discord.ButtonStyle.secondary, custom_id="open_schadensmeldung")
...
class InaktivitätsWarnView(discord.ui.View):
    ...
    async def cancel_auto_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not is_mitarbeiter(interaction) and interaction.user.id != cust_discord_id:
            await interaction.response.send_message(
                embed=build_error_embed("Zugriff verweigert!", "Nur Mitarbeiter oder der Versicherungsnehmer können das abbrechen."),
                ephemeral=True
            )
            return
...
class SchadensmeldungTicketView(discord.ui.View):
    ...
    @discord.ui.button(label="Beanspruchen", style=discord.ButtonStyle.primary,
                       custom_id="sd_claim_release")
    async def claim_release(self, interaction: discord.Interaction, button: discord.ui.Button):
        channel_key = str(interaction.channel.id)
        ticket_info = data.get("ticket_channels", {}).get(channel_key)

        if not ticket_info:
            await interaction.response.send_message(
                embed=build_error_embed("Fehler!", "Ticket-Daten nicht gefunden. Bitte wende dich an einen Administrator."),
                ephemeral=True
            )
            return
        ...
        if claimed_by is None:
            ...
            if not is_mitarbeiter(interaction):
                await interaction.response.send_message(
                    embed=build_error_embed("Zugriff verweigert!", "Nur Mitarbeiter können Tickets beanspruchen.", "Mitarbeiter"),
                    ephemeral=True
                )
                return
            ...
            for role in _get_roles(interaction.guild, MITARBEITER_ROLE_ID):
                await interaction.channel.set_permissions(role, read_messages=True, send_messages=False)
            await interaction.channel.set_permissions(
                interaction.user, read_messages=True, send_messages=True, read_message_history=True
            )
            ...
            add_log_entry("TICKET_BEANSPRUCHT", interaction.user.id, {"channel_id": interaction.channel.id})
...
class TicketCloseView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not is_mitarbeiter(interaction):
            ...
            return
        ch_key      = str(interaction.channel.id)
        ticket_info = data.get("ticket_channels", {}).get(ch_key, {"customer_id": self.customer_id})
        await close_ticket_channel(interaction, interaction.channel, ticket_info)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            for le_role in le_roles:
                overwrites[le_role] = discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True)
            for ma_role in ma_roles:
                # Mitarbeiter können mitlesen, aber nicht schreiben (außer dem Öffner)
                overwrites[ma_role] = discord.PermissionOverwrite(read_messages=True, send_messages=False, read_message_history=True)
            ...
            embed.add_field(name="Kontaktgrund", value=f"```{self.reason.value[:500]}```", inline=False)
            ...
            mentions = [interaction.user.mention] + ([customer_user.mention] if customer_user else [])
            await ticket_channel.send(" ".join(mentions), embed=embed, view=KundenkontaktTicketView())
            ...
            add_log_entry("TICKET_ERSTELLT", interaction.user.id, {"customer_id": customer_id, "channel_id": ticket_channel.id, "type": "kundenkontakt"})
            ...
            s = discord.Embed(title="Ticket erstellt!", description=f"Kundenkontakt-Ticket: {ticket_channel.mention}", color=EMBED_COLOR)
            await interaction.followup.send(embed=s, ephemeral=True)
        except Exception as e:
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if duplicate:
                ...
                dup_dt   = make_aware(datetime.fromisoformat(dup_date)).strftime("%d.%m.%Y %H:%M") if dup_date else "—"
                fraud_e  = dvg_embed(
                ...
                fraud_e.add_field(name="Nachweis",          value=f"> `{nachweis_val[:100]}`",  inline=False)
                fraud_e.add_field(name="Erstmalig eingereicht am", value=f"> `{dup_dt}`",       inline=True)
                fraud_e.add_field(name="Kunden-ID (original)",     value=f"> `{dup_cid}`",      inline=True)
                await interaction.followup.send(embed=fraud_e, ephemeral=True)
                add_log_entry("BETRUGSVERSUCH_ERKANNT", interaction.user.id, {
                    "nachweis": nachweis_val, "customer_id": customer_id,
            ...
            for le_role in le_roles:
                overwrites[le_role] = discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True)
            for ma_role in ma_roles:
                overwrites[ma_role] = discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True)
            if customer_user:
                overwrites[customer_user] = discord.PermissionOverwrite(read_messages=True, send_messages=True, read_message_history=True)
            ...
            embed.add_field(name="Nachweis / Rechnung", value=f"> `{nachweis_val}`", inline=False)
            embed.add_field(
                name="Bearbeitung",
            ...
            add_log_entry("TICKET_ERSTELLT", interaction.user.id, {
                "customer_id": customer_id, "channel_id": ticket_channel.id, "type": "schadensmeldung"
            })
            await send_to_dokumentation_channel(
...
async def add_user_to_ticket(interaction: discord.Interaction, user: discord.Member):
    ...
    if not (interaction.channel.name.startswith("kontakt-") or interaction.channel.name.startswith("schaden-")):
        await interaction.response.send_message(" Nur in Ticket-Channels nutzbar.", ephemeral=True)
        return
    await interaction.channel.set_permissions(user, read_messages=True, send_messages=True, read_message_history=True)
    e = discord.Embed(title="Person hinzugefügt!", description=f"> {interaction.user.mention} hat {user.mention} hinzugefügt.", color=EMBED_COLOR)
    e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
...
async def remove_user_from_ticket(interaction: discord.Interaction, user: discord.Member):
    ...
    e = discord.Embed(title="Person entfernt!", description=f"> {interaction.user.mention} hat {user.mention} entfernt.", color=EMBED_COLOR)
    e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
    await interaction.response.send_message(embed=e)
...
async def show_logs(interaction: discord.Interaction, anzahl: int = 10):
    ...
    try:
        if not data['logs']:
            await interaction.followup.send(embed=dvg_embed("Keine Logs vorhanden!"), ephemeral=True)
            return
        anzahl = max(1, min(anzahl, 25))
        recent = list(reversed(data['logs'][-anzahl:]))
        action_map = {
            "KUNDENAKTE_ERSTELLT": ("", "Kundenakte erstellt"), "RECHNUNG_ERSTELLT": ("", "Rechnung ausgestellt"),
            "RECHNUNG_ARCHIVIERT": ("", "Rechnung archiviert"), "MAHNUNG_1": ("", "1. Mahnung"),
            "MAHNUNG_2": ("", "2. Mahnung (+5%)"), "MAHNUNG_3": ("", "3. Mahnung (+10%)"),
            "TICKET_ERSTELLT": ("", "Ticket erstellt"), "TICKET_GESCHLOSSEN": ("", "Ticket geschlossen"),
            "TICKET_BEANSPRUCHT": ("", "Ticket beansprucht"), "TICKET_FREIGEGEBEN": ("", "Ticket freigegeben"),
            "AKTE_ARCHIVIERT": ("", "Akte archiviert"), "AUSZAHLUNG_EINGEREICHT": ("", "Auszahlungsantrag"),
            "AUSZAHLUNG_BESTAETIGT": ("", "Auszahlung bestätigt"), "AUSZAHLUNG_ABGELEHNT": ("", "Auszahlung abgelehnt"),
            "VERSICHERUNG_NACHGEBUCHT": ("", "Versicherung nachgebucht"), "VERSICHERUNG_GEKUENDIGT": ("", "Versicherung gekündigt"),
        }
        embed = discord.Embed(title="Aktivitätsprotokoll", description=f"**Letzte {len(recent)} Aktivitäten**", color=EMBED_COLOR, timestamp=get_now())
        for log in recent:
...
async def show_customer(interaction: discord.Interaction, customer_id: Optional[str] = None):
    ...
    if not mitarbeiter:
        found_id = next((cid for cid, c in data['customers'].items() if c.get('discord_user_id') == interaction.user.id and c.get('status') == 'aktiv'), None)
        if not found_id:
            await interaction.response.send_message(embed=build_error_embed("Keine Akte gefunden!", "Für Ihren Account wurde keine aktive Versicherungsakte gefunden.\nBitte wenden Sie sich an einen Mitarbeiter."), ephemeral=True)
            return
        customer_id = found_id
    else:
        if not customer_id:
            await interaction.response.send_message(embed=build_error_embed("Fehlende Angabe!", "Bitte geben Sie eine Versicherungsnehmer-ID an."), ephemeral=True)
            return
        if customer_id not in data['customers']:
            await interaction.response.send_message(embed=build_error_embed("Nicht gefunden!", f"Keine Akte mit ID `{customer_id}`."), ephemeral=True)
            return
    ...
    customer  = data['customers'][customer_id]
    ins_types = get_insurance_types()
    embed = discord.Embed(title=f" Kundenakte — {customer['rp_name']}", color=EMBED_COLOR, timestamp=get_now())
    embed.add_field(name="Stammdaten", value=f"> Kunden-ID: `{customer_id}`\n> RP-Name: **{customer['rp_name']}**\n> Kartenzahlung: `{customer['hbpay_nummer']}`\n> Economy-ID: `{customer['economy_id']}`\n> Status: `{customer.get('status','aktiv')}`", inline=False)
    ...
    embed.add_field(name="Versicherungen & Auszahlungslimits", value=ins_text or "> Keine", inline=False)

    off_re = len([inv for inv in data['invoices'].values() if inv['customer_id'] == customer_id and not inv.get('paid')])
    bez_re = len([inv for inv in data['invoices'].values() if inv['customer_id'] == customer_id and inv.get('paid')])
    embed.add_field(name="Rechnungsübersicht", value=f"> Offen: `{off_re}`\n> Bezahlt: `{bez_re}`\n> Monatsbeitrag: `{customer['total_monthly_price']:,.2f} €`", inline=False)
    embed.add_field(name="Metadaten", value=f"> Angelegt: {make_aware(datetime.fromisoformat(customer['created_at'])).strftime('%d.%m.%Y • %H:%M Uhr')}\n> Discord: <@{customer['discord_user_id']}>", inline=False)
    thread_id = customer.get("thread_id")
    if thread_id:
...
async def show_stats(interaction: discord.Interaction):
    ...
    total_mb   = sum(c.get('total_monthly_price', 0) for c in customers.values() if c.get('status') == 'aktiv')
    ...
    embed.add_field(name="Kundenstamm", value=f"> Aktiv: **`{aktive}`**\n> Archiviert: `{archiviert}`\n> Gesamt: `{len(customers)}`", inline=True)
    embed.add_field(name="Finanzen", value=f"> Monatsbeiträge: **`{total_mb:,.2f} €`**\n> Ausgezahlt: `{total_az:,.2f} €`", inline=True)
    embed.add_field(name="Rechnungen", value=f"> Offen: **`{off_re}`**\n> Bezahlt: `{bez_re}`", inline=True)
    # ── Bearbeitungszeiten ───────────────────────────────────────────────────
    stats_list    = data.get("ticket_stats", [])
    sm_stats      = [s for s in stats_list if s.get("type") == "schadensmeldung" and s.get("duration_minutes")]
    kk_stats      = [s for s in stats_list if s.get("type") == "kundenkontakt"   and s.get("duration_minutes")]
    def avg_min(lst): return int(sum(s["duration_minutes"] for s in lst) / len(lst)) if lst else None
    def fmt_min(m): return f"{m // 60}h {m % 60}min" if m else "—"
...
#   SCHADENHISTORIE
# ═══════════════════════════════════════════════════════
@bot.tree.command(name="schaden-historie", description="Zeigt die Schadenhistorie eines Versicherungsnehmers")
@app_commands.describe(customer_id="Versicherungsnehmer-ID")
@app_commands.autocomplete(customer_id=customer_id_autocomplete)
async def cmd_schaden_historie(interaction: discord.Interaction, customer_id: str):
    ...
    if not history:
        e.add_field(name="Keine Einträge", value="> Für diesen Kunden wurden noch keine Schadensmeldungen eingereicht.", inline=False)
    else:
        for entry in sorted(history, key=lambda x: x.get("datum",""), reverse=True)[:10]:
            datum     = make_aware(datetime.fromisoformat(entry["datum"])).strftime("%d.%m.%Y %H:%M") if entry.get("datum") else "—"
            status    = " Abgeschlossen" if entry.get("status") == "abgeschlossen" else " Offen"
            closed_dt = ""
...
#   PORTAL-ZUGANG
# ═══════════════════════════════════════════════════════
@bot.tree.command(name="portal-zugang", description="Sendet dem Kunden seinen persoenlichen Portal-Zugangscode per DM")
@app_commands.describe(customer_id="Versicherungsnehmer-ID")
@app_commands.autocomplete(customer_id=customer_id_autocomplete)
async def cmd_portal_zugang(interaction: discord.Interaction, customer_id: str):
    ...
    add_log_entry("PORTAL_ZUGANG_AUSGESTELLT", interaction.user.id,
...
async def auto_backup():
    try:
        ...
        e.add_field(name="Information", value="> Alle `24 Stunden` werden die Daten gesichert.", inline=False)
        e.add_field(name="Enthaltene Dateien", value="> - `insurance_data.json`\n> - `bot_config.json`", inline=False)
        e.add_field(name="Zeitstempel", value=f"> {get_now().strftime('%d.%m.%Y, %H:%M:%S Uhr')}", inline=False)
        e.set_footer(text=FOOTER_TEXT, icon_url=FOOTER_ICON)
        for guild in bot.guilds:
            ch = guild.get_channel(config["log_channel_id"])
            if ch:
            ...
            async for msg in ch.history(limit=20):
                if msg.author.id == bot.user.id and msg.embeds:
                    ...
                    if updated:
                        ...
                        logger.info("Schadensmeldungs-Panel Bearbeitungszeit aktualisiert.")
                    break
            break
    except Exception as ex:
...
async def on_message(message: discord.Message):
    ...
    if ch_key in data.get("ticket_channels", {}):
        data["ticket_channels"][ch_key]["last_human_message"]  = get_now().isoformat()
        data["ticket_channels"][ch_key]["inactivity_warned_at"] = None  # Timer zurücksetzen
        save_data(data)
    await bot.process_commands(message)
...
def portal_dashboard_route(token: str):
    ...
    if not cid:
        return redirect('/portal?error=1')
    return build_portal_dashboard(cid, customer, token), 200, \
...
if __name__ == "__main__":
    ...
    if not token:
        logger.error("INSURANCE_TOKEN nicht gefunden! Bitte als Umgebungsvariable setzen.")
    else:
